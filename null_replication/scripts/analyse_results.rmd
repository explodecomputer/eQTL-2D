...
- the replication rate could be quite high using FDR
...
```{r}
ggplot(res, aes(x=varexp, y=nsig_disc_bonf)) +
geom_point()
```
...
group_by(res, lambda_disc_bin) %>% summarise(n=mean(nsig_disc_bonf, na.rm=T)) %>% 
ggplot(aes(x=lambda_disc_bin, y=n)) +
geom_point() +
theme(axis.text.x=element_text(angle=90, hjust=0.5, vjust=0.5)) +
labs(x="Simulation's lambda", y="Mean number of discovery hits")
```
...
theme(axis.text.x=element_text(angle=90, hjust=0.5, vjust=0.5)) +
labs(x="Simulation's lambda", y="Replication rate")
```
