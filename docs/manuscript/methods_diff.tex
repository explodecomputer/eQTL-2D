...
%DIF DEL methods.tex   Wed Sep  4 00:07:21 2013
%DIF ADD methnew.tex   Wed Sep  4 00:04:44 2013
\usepackage{url}
...
\begin{equation}
G\DIFdelbegin \DIFdel{_{ijk} }\DIFdelend \DIFaddbegin \DIFadd{_{jk} }\DIFaddend = \left \{ 
\DIFdelbegin %DIFDELCMD < \begin{array}{ll}
%DIFDELCMD < \sigma _a ^2 + \sigma _e ^2&        j = k \\ 
%DIFDELCMD < 2\phi _{ijk} \sigma _a ^2& 			j \neq k \\
%DIFDELCMD < \end{array} %%%
...
\end{equation}
The \DIFdelbegin \DIFdel{parameters }\DIFdelend \DIFaddbegin \DIFadd{parameter }\DIFaddend $\sigma_a^2$ \DIFdelbegin \DIFdel{and $\sigma_e^2$ are variance components }\DIFdelend \DIFaddbegin \DIFadd{is the variance component }\DIFaddend for additive background genetic\DIFdelbegin \DIFdel{and environmental effects respectively}\DIFdelend . Here, we are using family based pedigree information rather than SNP based IBD to account for relationships between individuals and so \DIFdelbegin \DIFdel{$\phi _{ijk}$ }\DIFdelend \DIFaddbegin \DIFadd{$\phi _{jk}$ }\DIFaddend is the kinship coefficient between individuals $j$ and $k$. The residual, $e$, from equation \ref{eq:lm} is assumed to follow a multivariate normal distribution with a mean of zero. Residuals were normalised by rank transformation and used as the adjusted phenotype for the pairwise epistasis scan to remove any skewness and avoid results being driven by outliers. The GenABEL package for R was used to perform the normalisation \cite{Aulchenko2007}.
...
\DIFdelbegin \DIFdel{Following filtering on this threshold }\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Filtering}}

\DIFadd{We used two approaches to filter SNPs from stage 1 to be tested for significant interaction effects in stage 2.
}

\paragraph{\DIFadd{Filter 1}} \DIFadd{After keeping SNP pairs that surpassed the $2.91 \times 10^{-16}$ threshold in stage 1 }\DIFaddend only SNP pairs with at least 5 data points in all 9 genotype classes were kept. We then calculated the LD between interacting SNPs (amongst unrelated individuals within the discovery sample and also from 1000 genomes data) and removed any pairs with $r^2 > 0.1$ or $D'^{2} > 0.1$ to avoid the inclusion of haplotype effects and to increase the accuracy of genetic variance decomposition. If multiple SNP pairs were present on the same chromosomes for a particular expression trait then only the sentinel SNP pair was retained, \emph{i.e.} if a probe had multiple SNP pairs that were on chromosomes one and two then only the SNP pair with the most significant $p$-value was retained. At this stage 6404 filtered SNP pairs remained. 
\DIFdelbegin \DIFdel{We then }\DIFdelend \DIFaddbegin 

\paragraph{\DIFadd{Filter 2}} \DIFadd{We also }\DIFaddend performed a second filtering screen \DIFaddbegin \DIFadd{applied to the list of SNP pairs from stage 1 }\DIFaddend that was identical to \DIFdelbegin \DIFdel{the first }\DIFdelend \DIFaddbegin \DIFadd{filter 1 }\DIFaddend but an additional step was included where any SNPs that had previously been shown to have a significant additive or dominant effect ($p < 1.29 \times 10^{-11}$) was removed \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Powell2013}
}%DIFAUXCMD
}\DIFaddend , creating a second set of 4751 \DIFaddbegin \DIFadd{unique }\DIFaddend filtered SNP pairs.

\DIFaddbegin \subsubsection{\DIFadd{Stage 2}}
...
\paragraph{} \DIFadd{The probability of observing the number of concordant signs in tests 1-3 is calculated using a binomial test. All variance decompositions were calculated using the NOIA method \mbox{%DIFAUXCMD
\cite{Alvarez-Castro2008}
}%DIFAUXCMD
.
}
...
\DIFaddend \section{Additive and non-additive variance estimation}
