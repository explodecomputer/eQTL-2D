...
We tested for enrichment of genic features for the 731 SNPs that comprise the 501 epistatic interactions. For each of the 731 SNPs we calculated LD with all neighbouring SNPs within a radius of 1kb and kept all neighbouring SNPs with LD $r^2 > 0.8$. We then cross-referenced the remaining regional SNPs with the annotated reference genome (version HG19) querying whether the regional SNPs fell in intergenic, promotor, intronic, exonic, splice sites, 5' UTR or 3' UTR positions. Therefore a particular query SNP might have regional SNPs in multiple genic features.

We then performed the whole querying process for each of the 528,509 SNPs present in the SNP chip used in the scan, and used the results from this second analysis to establish the expected proportion of SNPs for each genic feature.
