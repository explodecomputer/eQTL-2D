...
We tested for enrichment of genomic features for the 687 IndexSNPs that comprise the 434 epistatic interactions with data present in discovery and replication datasets. For each of the 687 IndexSNPs we calculated LD with all regional SNPs within a radius of 0.5Mb and kept all regional SNPs with LD $r^{2} > 0.8$. We then cross-referenced the remaining regional SNPs with the annotated reference genome (version HG19, GENCODE annotation version 16 \cite{Harrow2012}) querying whether the regional SNPs fell in intergenic, promotor, intronic, exonic, splice sites, 5' UTR or 3' UTR positions. Therefore a particular IndexSNP might cover multiple genomic features.
