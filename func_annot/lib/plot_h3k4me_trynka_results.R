...
bindMe <- function(tis_score) {
  ...
  tiss_score <- read.table(str_c("DATA/H3K4me3_output_final/", tis_score),  sep = "\t", header = TRUE) 
...
files <- dir(path = "DATA/H3K4me3_output_final/", pattern = "levis", recursive = TRUE)
names(files) <- str_split_fixed(files, "\\.", n = 2)[ ,1]

# Drop the all-SNPs because we decided to provide only cis/trans subsets
# given the all-SNPs list analysis look pretty mush as cis-
files <- files[2:3]
...
levels(all_tiss_scores$samp_ind) <- c("cis SNPs (324)", "trans SNPs (454)")
