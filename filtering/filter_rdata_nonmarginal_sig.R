...
snplistfile <- commandArgs(T)[9]
threshold2 <- as.numeric(commandArgs(T)[10])
...
load(snplistfile)
dim(marginal_list)
marginal_list <- subset(marginal_list, pval < 10^-threshold2)
dim(marginal_list)


# Filter so that any SNPs in snplist for a particular probe are removed
marginal_list <- subset(marginal_list, probename == res$probename[i])
dim(marginal_list)
...
# Choose the largest pint for each chromosome pair
res <- res[order(res$chr1, res$chr2, res$pint, decreasing=T), ]
